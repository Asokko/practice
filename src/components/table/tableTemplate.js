...
export function createTable(rowsCount=20, state={}){
    ...
    const cols =new Array(colsCount).fill('') 
        .map(toChar)            //.map((col,index)=>{
        .map(widhtFrom(state))  //const width=getWidth(state.colState, index)
        .map(toColumn)          // return toColumn(col, index, width)
        .join('')               //})
    rows.push(createRow(null, cols,{}))

    for (let row=0; row<rowsCount; row++){
        const cells =new Array(colsCount).fill('')
        .map(toCell(state, row)) //.map((_, col)=>toCell(row,col))
        .join('')
